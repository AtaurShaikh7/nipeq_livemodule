PROCEDURE SP_FE_LIVE_13_06 (FUNDID in number,RUN_DATE in Date,REP_TYPE in VARCHAR,ResultSet out SYS_REFCURSOR)
...
BEGIN
  ...
  if (CHECK_EXIST <> 0) then
     ...
     open ResultSet for 
         ...
         CASE WHEN (rep_type <> 'LIVE') THEN nvl(ROUND(PD.RET_1D,8),0) ELSE 0 END AS RET_1D,

         nvl(ROUND(PD.RET_5D,8),null) AS RET_5D,

         nvl(ROUND(PD.RET_1M,8),null) AS RET_1M,

         nvl(ROUND(PD.RET_3M,8),null) AS RET_3M,

         nvl(ROUND(PD.RET_6M,8),null) AS RET_6M,

         nvl(ROUND(PD.RET_1Y,8),null) AS RET_1Y,
