PROCEDURE SP_FE_LIVE (FUNDID in number, INDEXID in number ,RUN_DATE in Date,REP_TYPE in VARCHAR,ResultSet out SYS_REFCURSOR, LOGINID IN VARCHAR)
...
BEGIN
  ...
  --EFFECT_DATE := '07-may-2018';

    INSERT INTO USERSACCESS_LOG (LOGIN_ID,ACTIVITY_DATE,ACTIVITY,PAGE_NAME)

   VALUES (LOGINID,SYSDATE,'Live Report Created for : '||FUNDID||':'||INDEXID||':'||RUN_DATE, 'Live Report');
  ...
  if (CHECK_EXIST <> 0) then
     ...
     open ResultSet for 
         ...
         select UPPER(Sect.SECTOR_SHORT_NAME) as SECTOR, substr(SM.SECURITY_NAME,1,60) as SECURITY_NAME,
         ...
         CASE WHEN ALLD.FUND_WT is NULL THEN 'No' ELSE 'FUND' END AS FundFlag, 

         ALLD.FUND_QTY as FUNDQTY,

         nvl(ALLD.closep, NVL(PD.CLOSEP,0)) as CMP ,

         nvl(ROUND(PD.RET_1D,8),0) AS RET_1D,

         nvl(ROUND(PD.RET_5D,8),0) AS RET_5D,

         nvl(ROUND(PD.RET_1M,8),0) AS RET_1M,

         nvl(ROUND(PD.RET_3M,8),0) AS RET_3M,

         nvl(ROUND(PD.RET_6M,8),0) AS RET_6M,

         nvl(ROUND(PD.RET_1Y,8),0) AS RET_1Y,

         nvl(ROUND(PD.RET_YTD,8),0) AS RET_YTD,
         ...
         ROUND(FUND_AUM/10000000,0) as FUND_AUM, SR_PD.MARKETCAP as MCAP,
        ...
        case when SSM_SUBSECT.SECTOR_ID > 100 THEN 0 ELSE 1 END AS NO_SUBSEC

        ,NVL(SDF.MARKETCAP_BUCKET,'NA') AS MCAP_BUCKET

        ,NVL(AV.AVGVOLUME_3M,0) AS AVG_VOL

        ,NVL(AV.AVGAVDT_3M,0) AS AVGADVT

         ,nvl(security_rating_intmdt.Rating,'NA') as Rating
      ...
      LEFT JOIN 
         ...
         on ALLD.security_code=su_sp.security_code 
       ...
       LEFT JOIN         
         ...
         (SELECT * FROM SECURITY_RETURNS WHERE TRUNC(EFFECTIVE_DATE)=TRUNC(EFFECT_DATE)) SR_PD

         ON (SR_PD.SECURITY_CODE=   case when su_sp.underlier_code is null then ALLD.SECURITY_CODE else su_sp.underlier_code end )
        ...
        security_code_bbisin_intmdt MAPP on (SM.SECURITY_CODE=MAPP.BM_CODE)
         ...
         FUNDS F ON F.FUND_ID = FUNDID

         INNER JOIN

         SECURITY_SECTOR_MAPPING SSM_SECT

         ON SSM_SECT.SECTOR_CLASS_ID = f.def_sector_class_id AND SSM_SECT.SECURITY_CODE = ALLD.SECURITY_CODE

         INNER JOIN

         (SELECT * FROM SECURITY_SECTOR_MAPPING WHERE SECTOR_CLASS_ID = 3) SSM_SUBSECT

         ON SSM_SUBSECT.SECURITY_CODE = ALLD.SECURITY_CODE

         INNER JOIN

         Sectors sect on (SSM_SECT.Sector_ID=Sect.Sector_id)

         Inner join 

         Sectors SubSect on (SSM_SUBSECT.Sector_ID = SubSect.Sector_id)

         LEFT JOIN 

         (

            SELECT SECURITY_CODE,EFFECTIVE_DATE,MARKETCAP_BUCKET FROM SECURITY_DYNAMIC_FACTORS WHERE TRUNC(EFFECTIVE_DATE) = TRUNC(EFFECT_DATE)

         )SDF

          ON (sdf.SECURITY_CODE= case when su_sp.underlier_code is null then ALLD.SECURITY_CODE else su_sp.underlier_code end )

         LEFT JOIN

         (

          SELECT * FROM AVERAGE_VOLUME WHERE EXCHANGE_ID = 1

         ) AV

         ON AV.SECURITY_CODE = ALLD.SECURITY_CODE AND AV.EFFECTIVE_DATE = TRUNC(EFFECT_DATE)

          left  Join 

         (SELECT DISTINCT SECURITY_CODE, RATING FROM security_rating_intmdt) security_rating_intmdt

     on (security_rating_intmdt.SECURITY_CODE= case when su_sp.underlier_code is null then ALLD.SECURITY_CODE else su_sp.underlier_code end )
         ...
         null AS IndexFlag,CASE WHEN SUM(NVL(ALLD.FUND_WT,0)) > 0 THEN 'FUND' ELSE null END AS FundFlag, 
         ...
         case when sum(NVL(ALLD.FUND_WT,0)) = 0 then null else Round((sum(NVL(PD.RET_1M,0) * NVL(ALLD.FUND_WT,0))/sum(NVL(ALLD.FUND_WT,0)))*100,1) end as RET_1M,

         case when sum(NVL(ALLD.FUND_WT,0)) = 0 then null else Round((sum(NVL(PD.RET_3M,0) * NVL(ALLD.FUND_WT,0))/sum(NVL(ALLD.FUND_WT,0)))*100,1) end as RET_3M,

         case when sum(NVL(ALLD.FUND_WT,0)) = 0 then null else Round((sum(NVL(PD.RET_6M,0) * NVL(ALLD.FUND_WT,0))/sum(NVL(ALLD.FUND_WT,0)))*100,1) end as RET_6M,

         case when sum(NVL(ALLD.FUND_WT,0)) = 0 then null else Round((sum(NVL(PD.RET_1Y,0) * NVL(ALLD.FUND_WT,0))/sum(NVL(ALLD.FUND_WT,0)))*100,1) end as RET_1Y,
