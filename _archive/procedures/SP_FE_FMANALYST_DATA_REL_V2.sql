PROCEDURE SP_FE_FMANALYST_DATA_REL_V2 (RUN_DATE in Date,LOGINID in VARCHAR2,SORTBYCOL in VARCHAR2, ResultSet out SYS_REFCURSOR)
...
BEGIN
    ...
    Insert into FM_ANL_DATA (FUND_NAME,FM_NAME,IDX_NAME,SECURITY_NAME,SECURITY_CODE,SECTOR_NAME,TOTALQTY,TOTALINVEST,CMP,MCAP,FUNDWGT,MARKETVALUE,SHARES,OU,AUM,ANALYST,RET1D,RET5D,RET1M,RET3M,RET6M,RET1Y,RETYTD,SECURITY_TYPE,BV,PERCHANGE,mcap_diff)
    ...
    ,case when nvl(sr.mcap,0) <>0 then round((nvl(fhm.mtm,0)/sr.mcap)*100,2) else 0 end --added by dhwani calculating os Equity 

    from

    ( 

      select fund_id, fhs.security_code, quantity, mtm_value,mtm, FundWgt, TotalQty,TotalInv, AUM,BV,PerChange from
      ...
      (
        ...
        select security_code, to_char(sum(quantity),'99,99,99,99,99,99,999') as TotalQty

        ,to_char(sum(mtm_value)/10000000,'99,99,99,99,99,999') as TotalInv

        ,sum(mtm_value)/10000000 as Mtm --added by dhwani for fetch raw mtm value without comma
    ...
    to_char(marketcap,'99,99,99,99,99,999') as marketcap

    ,marketcap as mcap,--added by dhwani for fetch raw mcap value without comma

    to_char(closep,'99,99,99,99,99,999') as closep,security_code,effective_date
...
END SP_FE_FMANALYST_DATA_REL_V2;