PROCEDURE SP_FE_HISTPERF_PEERS_REL (FUNDID in number,RUN_DATE in Date,REP_TYPE in VARCHAR,ResultSet out SYS_REFCURSOR)
...
select Fund_NAME,FUND_NAV,RET_1M,RET_3M,RET_6M,RET_1Y,RET_3Y,RET_5Y,AUM from(

select F.Short_Name as Fund_Name,nvl(FNR.nav_value,NVL(FN.NAV_PER_UNIT,0)) as FUND_NAV,

NVL(FNR.ret_1m,0) as RET_1M,

NVL(FNR.ret_3m,0) as RET_3M,

NVL(FNR.ret_6m,0) as RET_6M,

NVL(FNR.ret_1y,0) as RET_1Y,

NVL(FNR.ret_3y,0) as RET_3Y,

NVL(FNR.ret_5y,0) as RET_5Y,
...
select I.INDEX_SHORT_NAME as Fund_Name,nvl(nvl(INR.closing_value,IP.Closep),0) as FUND_NAV,

nvl(INR.ret_1m,0) as RET_1M,

nvl(INR.ret_3m,0) as RET_3M,

nvl(INR.ret_6m,0) as RET_6M,

nvl(INR.ret_1y,0) as RET_1Y,

nvl(INR.ret_3y,0) as RET_3Y,

nvl(INR.ret_5y,0) as RET_5Y,
...
select  F.fund_name as Fund_Name,nvl(PNR.nav_value,0) as FUND_NAV,

nvl(PNR.ret_1m,0) as RET_1M,

nvl(PNR.ret_3m,0) as RET_3M,

nvl(PNR.ret_6m,0) as RET_6M,

nvl(PNR.ret_1y,0) as RET_1Y,

nvl(PNR.ret_3y,0) as RET_3Y,

nvl(PNR.ret_5y,0) as RET_5Y,

nvl(PNR.AUM,0) AS AUM
